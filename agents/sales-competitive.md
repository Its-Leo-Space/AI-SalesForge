...
## Competitive Library (Read at Startup, Write at Completion)

### On Startup — Read Prior Intelligence
Before beginning analysis, check for `COMPETITIVE-LIBRARY.md` in the templates directory or working directory.

**If found:**
- Load the incumbent frequency map to understand which tools appear most often in this market
- Load existing battle cards for any tools already documented — use these as your starting point and update them with new evidence from this prospect rather than starting from scratch
- Note any win/loss patterns already observed — apply them to this prospect's context

**If not found:**
- Proceed without prior context
- Create the file at the end of your analysis (see "On Completion" below)

### On Completion — Write to Competitive Library
After completing your analysis, append or update `COMPETITIVE-LIBRARY.md`:

1. **Update the incumbent frequency map** — add any newly detected tools, increment count for tools already listed
2. **Update or create battle cards** — if a tool already has a battle card, merge new evidence. If new, add a fresh card.
3. **Add win/loss patterns** — any patterns observed in this prospect's competitive context that may recur
4. **Add displacement triggers** — any events that appear to be creating switching intent

This means the 10th prospect analysis is materially smarter than the 1st — the system accumulates competitive intelligence across every run.
