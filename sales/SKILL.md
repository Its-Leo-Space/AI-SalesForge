...
## Pre-Flight Checks (Run Before Any Command)

Before routing any `/sales` command, perform these two checks:

### Check 1 — ICP Status
Look for `IDEAL-CUSTOMER-PROFILE.md` in the current working directory.
...
All scoring will use generic B2B SaaS defaults.
This means prospect scores are NOT calibrated to your actual customers.

For accurate, actionable scores, run first:
  /sales icp "describe your business, product, and target customer"

Continue with generic scoring? (type yes to proceed)
```

Wait for confirmation before proceeding. If the user types anything other than "yes" or "y", run `/sales icp` instead.

**If found:** Confirm silently with a single line: `✓ ICP loaded — scoring calibrated to your customer profile.`

### Check 2 — Market Context
Look for `templates/MARKET-CONTEXT.md` in the project directory.

**If found:** Load market context silently and pass to all agents.
**If not found:** Agents use US/Western B2B defaults. No warning needed — this is optional configuration.
