...
### Automatic File Intelligence (Read Before Asking)

Before asking the user for any context, check for these files and extract data automatically:

**`MEETING-PREP.md`** (highest priority — read first):
If this file exists, extract and use:
- **Success Metrics section**: What was the intended outcome of the meeting? Use this to determine whether the meeting "went well" — if the target success was "agree to a technical demo" and no demo was scheduled, the follow-up needs to re-open that conversation.
- **Next Steps to Propose section**: What did the prep plan to ask for? If it was the Bold or Standard ask, anchor the follow-up around whether that happened.
- **Objections to Expect section**: Which objections appeared in the prep? Assume one or more came up and address them proactively in the follow-up sequence.
- **Cheat Sheet — Trap to Avoid**: Carry this forward — do not reference the same topics flagged as landmines.
- **Discovery Questions section**: Which questions were planned? Use the answers (if known) or the questions themselves to inform Email 1 content.

**`PROSPECT-ANALYSIS.md`**: Use for company context, pain points, and contact data.
**`LEAD-QUALIFICATION.md`**: Use for BANT context and deal stage assessment.
**`DECISION-MAKERS.md`**: Use for contact profiles and personalization anchors.
**`OUTREACH-SEQUENCE.md`**: Check if this is a post-first-sequence follow-up — if so, ensure Email 1 of the follow-up acknowledges prior contact.
**`OBJECTION-PLAYBOOK.md`**: If objections from the meeting are known, use the playbook's word-for-word scripts in the follow-up rather than generating new responses.

After reading available files, display what was found:
```
CONTEXT LOADED FROM FILES:
  Meeting Prep:      [Found / Not found]
  Intended outcome:  [Extracted from Meeting Prep or "Unknown"]
  Next step planned: [Extracted or "Unknown"]
  Objections prepped: [List or "Unknown"]
  Prospect files:    [List of files found]
```

Only ask the user for information NOT available from these files.
