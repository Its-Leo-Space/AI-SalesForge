...
- **OBJECTION-PLAYBOOK.md**: If exists, extract likely objections and use the word-for-word scripts from the playbook in the proposal narrative — these are specific to this prospect/industry and will be more credible than generic objection handling

### ROI-BENCHMARKS.md (Critical for Proposal Credibility)

Check for `templates/ROI-BENCHMARKS.md` in the project directory.

**If found — Evidenced ROI mode:**
- Read all case studies, product metrics, and industry benchmarks
- Use ONLY the figures documented in this file for ROI projections
- Cite the source for every number: "(Source: Case Study 2, Q3 2025)"
- Any projection that cannot be backed by a ROI-BENCHMARKS.md entry must be flagged inline: "⚠️ Estimated — no benchmark data available. Replace with real data before sending."
- The proposal is only as credible as its ROI evidence — flagged estimates are honest; invented figures are a credibility risk.

**If NOT found — Estimated ROI mode:**
- Add a notice at the top of Section 8 (ROI Projection): "⚠️ ROI figures below are illustrative estimates. Populate templates/ROI-BENCHMARKS.md with real customer data before using this proposal with prospects."
- Use conservative, clearly-labeled estimates
- Structure the estimates so the client can validate them with their own numbers: "If your team spends [X hours] per week on [the problem] at [their hourly rate], your current cost is [calculated amount]."

If previous files exist, inform the user what data was found and incorporate it automatically. Only ask for inputs that are not available from existing files.
