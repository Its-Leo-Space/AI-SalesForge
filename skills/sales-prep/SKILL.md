...
7. **If `OBJECTION-PLAYBOOK.md` exists**, read it and use it directly for Section 7 (Objections to Expect). Specifically: extract the industry-specific objections (Section 16-20 of the playbook), the competitive objections relevant to this prospect's detected stack, and any pricing objections. Use the word-for-word scripts from the playbook verbatim — do NOT generate new scripts that may conflict. If no playbook exists, generate objection responses using the psychological architectures (LQS, MIC, AAR, FCA) from the objection skill.
8. **Time-sensitive accuracy.** Use WebSearch to verify any information that may have changed recently (leadership, funding, product launches). Note the date of each source.
9. **The Cheat Sheet is the most important section.** If the salesperson reads nothing else, the Cheat Sheet alone should make them meaningfully more prepared than walking in blind.
10. **Discovery questions must be genuinely curious.** They should be questions the salesperson actually wants to know the answer to — not leading questions designed to manipulate the prospect into a predetermined conclusion.
11. **Competitor references must be factual.** Never fabricate competitive intelligence. If a competitor's weakness cannot be verified, label it as "commonly reported" or omit it.
