...
## Data Quality Gate (Run Before Any Analysis)

Before beginning synthesis, assess the data quality from each of the other four agents. This gate determines what you can and cannot produce.

```
DATA QUALITY ASSESSMENT

Contact Access Score received: [X]/100
  < 20 → FLAG: "Insufficient contact data"
        ACTION: Do NOT draft a personalised email.
        OUTPUT: A research checklist of what to find manually before outreach.
        Checklist must include: team page URLs to check, LinkedIn search queries 
        to run, email patterns to test, and specific roles to identify.

Opportunity Quality Score received: [X]/100
  < 25 → FLAG: "Unconfirmed need"
        ACTION: Shift email framework from Trigger Event to Problem→Proof.
        Do not reference specific pain points as confirmed — frame as hypotheses.
        Add a note in output: "Pain points are inferred, not confirmed. 
        Prioritise discovery over pitching in first contact."

Company Fit Score received: [X]/100
  < 30 → FLAG: "Potential ICP mismatch"
        ACTION: Add warning to output: "Company fit is below threshold. 
        Recommend running /sales qualify before investing in a full outreach sequence.
        Outreach Readiness score capped at 50 when Company Fit < 30."

All three scores ≥ thresholds → Proceed with full analysis.
```

Display data quality status before your analysis:
```
DATA QUALITY STATUS
  Contact Data:    [Sufficient / Insufficient — research checklist below]
  Need Confirmed:  [Yes / Inferred — framework adjusted]
  ICP Fit:         [Strong / Weak — score capped / proceed]
```
