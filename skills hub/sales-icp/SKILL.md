...
- **Invocation:** `/sales icp <description>` — build or rebuild ICP from scratch
- **Invocation:** `/sales icp refine` — learn from prospect history and sharpen existing ICP
- **Input:** A description of the user's business, product, or service (for build) OR existing PROSPECT-ANALYSIS.md files (for refine)
...
### Mode 1: Build (default — when a description is provided)
Run the full ICP generation process described below.

### Mode 2: Refine (`/sales icp refine`)
Run when the user types `/sales icp refine` — no description needed.

**Refine process:**

**Step 1 — Scan for all prospect analysis files:**
Read every `PROSPECT-ANALYSIS.md`, `LEAD-QUALIFICATION.md`, and `COMPANY-RESEARCH.md` in the current directory. Minimum 3 files required; if fewer exist, tell the user to run more prospect analyses first.

**Step 2 — Extract and compare patterns across all prospects:**
For each prospect file, extract:
- Company type, size, industry, and stage
- Prospect Score and grade
- Which scoring dimensions were strongest and weakest
- Pain points detected with severity
- BANT dimension scores
- Trigger events that scored highest
- Contact access quality

**Step 3 — Identify what high-scoring prospects have in common:**
Find the characteristics shared by all prospects scoring 70+:
- Industry verticals appearing most often
- Company size ranges that scored best
- Tech stack signals correlated with high scores
- Pain points that appeared across multiple high-scoring prospects
- Trigger events that appeared in high-scoring prospects

**Step 4 — Identify what low-scoring prospects have in common:**
Find the characteristics shared by all prospects scoring below 50:
- Industries that consistently score poorly
- Size ranges that don't qualify well
- Anti-ICP signals that appear repeatedly

**Step 5 — Generate a refined ICP:**
Produce an updated `IDEAL-CUSTOMER-PROFILE.md` with:
- Adjusted firmographic criteria based on observed high-scoring patterns
- Updated scoring weights (dimensions where high-scoring prospects consistently score well get slightly higher weight)
- New anti-ICP signals identified from low-scoring prospects
- A "Pattern Intelligence" section documenting what was learned from the prospect history

**Step 6 — Show the diff:**
Display a clear before/after comparison showing what changed in the ICP and why:
```
ICP REFINEMENT SUMMARY
Based on [X] prospect analyses

Changed:
  Size range: 50-500 → 75-300 employees (high scorers cluster here)
  Added to anti-ICP: Healthcare (3/3 healthcare prospects scored < 40)
  Updated scoring weight: Growth trajectory 20% → 25% (strongest predictor of A-grades)
  New signal added: Recent Series A/B within 12 months (present in 4/5 A-grade prospects)

Unchanged:
  Industry fit, geographic criteria, tech sophistication threshold
```
